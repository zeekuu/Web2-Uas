...
@section('content')
    <div class="container">
            ...
            <div class="row mb-3">
                <div class="col-md-11">
                    <h4 class="fw-bold text-dark">FORM EDIT EVENT <span
                            class="text-primary fw-bold">{{ $event->namaEvent }}</span></h4>
                </div>
                <div class="col-md-1">
            ...
            @if ($event->status === 'rejected')
                    ...
                    <div class="col-md-12 mt-3">
                        <div class="alert alert-danger">
                            <strong>ACARA DIBATALKAN</strong>
                        </div>
                    </div>
                @endif
            <form action="{{ route('panitia.event.update', $event->idEvent) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('namaEvent') is-invalid @enderror" id="namaEvent"
                        name="namaEvent" value="{{ old('namaEvent') ?? $event->namaEvent }}">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi"
                        rows="4">{{ old('deskripsi') ?? $event->deskripsi }}</textarea>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control @error('tanggalEvent') is-invalid @enderror"
                            id="tanggalEvent" name="tanggalEvent" value="{{ old('tanggalEvent') ?? $event->tanggalEvent }}">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('waktuEvent') is-invalid @enderror" id="waktuEvent"
                            name="waktuEvent" value="{{ old('waktuEvent') ?? $event->waktuEvent }}"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('tempatEvent') is-invalid @enderror" id="tempatEvent"
                        name="tempatEvent" value="{{ old('tempatEvent') ?? $event->tempatEvent }}">
                ...
                <div class="row">
                    <div class="col-md-2 mb-3">
                        ...
                        <input type="number" class="form-control @error('kuotaPeserta') is-invalid @enderror"
                            id="kuotaPeserta" name="kuotaPeserta" value="{{ old('kuotaPeserta') ?? $event->kuotaPeserta }}"
                            min="1">
                        @error('kuotaPeserta') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-5 mb-3">
                        <label for="hargaTiket" class="form-label fw-bold small text-muted">HARGA TIKET (Isi 0 jika
                            Gratis)</label>
                        <input type="int" class="form-control @error('hargaTiket') is-invalid @enderror" id="hargaTiket"
                            name="hargaTiket" value="{{ old('hargaTiket') ?? $event->hargaTiket }}">
                    ...
                    <div class="col-md-5 mb-3">
                        ...
                        <input type="text" class="form-control @error('rekening') is-invalid @enderror" id="rekening"
                            name="rekening" value="{{ old('rekening') ?? $event->rekening }}">
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        <label for="poster" class="form-label fw-bold small text-muted">UPLOAD POSTER EVENT (Maks
                            2MB)</label>
                        <input type="file" class="form-control @error('poster') is-invalid @enderror" id="poster"
                            name="poster">
                        @if($event->poster)
                            File saat ini: <a href="{{ asset('storage/poster/' . $event->poster) }} "
                                class="btn btn-sm btn-primary mt-2">Lihat Poster</a>
                        @endif
                        @error('poster') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6">
                        <label for="fileProposal" class="form-label fw-bold small text-muted">UPLOAD FILE PROPOSAL (Maks
                            2MB)</label>
                        <input type="file" class="form-control @error('fileProposal') is-invalid @enderror"
                            id="fileProposal" name="fileProposal">
                        @if($event->fileProposal)
                            File saat ini: <a href="{{ asset('storage/proposal/' . $event->fileProposal) }}"
                                class="btn btn-sm btn-primary mt-2">Lihat Proposal</a>
                        @endif
